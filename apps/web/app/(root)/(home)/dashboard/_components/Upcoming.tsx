"use client";
import { MeetingMetadataType } from "@infinityMeet/types";
...
export default function Upcoming({ meeting }: { meeting: MeetingMetadataType[] }) {
    ...
    const filteredMeetings = meeting?.filter(meeting =>
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="ml-2 flex-shrink-0">
                                        <span className="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-white/4">
                                            {meeting.pariticipantsCounts} Participants
                                        </span>
                                    </div>
                                </div>

                                <p className="text-sm opacity-80 mb-4">{meeting.description || "Meeting description not available."}</p>

                                <div className="flex items-center text-sm opacity-70 mb-4">
                                    <Calendar size={14} className="mr-2" />
                                    <span>{meeting.startedAt ? formatDate(meeting.startedAt) : '---'}</span>
                                    <Clock size={14} className="ml-3 mr-2" />
                                    <span>{meeting.startedAt ? formatTime(meeting.startedAt) : '---'}</span>
                                </div>
                            </div>
                                    ...
                                    <tr key={meeting.id} className="border-b transition-colors">
                                        ...
                                        <td className="p-4 hidden sm:block">
                                            <div className="flex -space-x-2">
                                                {Array.from({ length: Math.min(3, meeting.pariticipantsCounts ?? 0) }).map((_, i) => (
                                                ...
                                                {(meeting.pariticipantsCounts ?? 0) > 3 && (
